package etechits.co.controller;
...
import etechits.co.controller.MemberController;
import etechits.co.service.AdminService;
...
public class AdminController {
    ...
    public ModelAndView admin_login(String admin_id, String admin_pass,HttpSession session) throws Exception    {
        ...
        if(result)    {//로그인이 성공했을시 출력되는 구문
            mav.setViewName("/admin/admin_member_forced_eviction_view");    //로그인이 성공했을시 이동하게되는 뷰의 이름
    ...
    public ModelAndView admin_member_forced_eviction(String userId) throws Exception {
        ...
        if(vo.getUserId() != null) {    //회원 강제탈퇴가 성공했을시 출력되는 뷰
